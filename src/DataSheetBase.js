export default class DataSheetBase {
  ...
  expandQueryStringWithSlotValues(query, slots) {
    // this method parses the tiny template format offered in React Studio's data sheet settings.
    // the query template strings can contain references to data slot values, like this:
    //
    ...
    while (scanIndex < query.length) {
      ...
      if (index !== -1) {
        let slotRefStr = query.substr(scanIndex, index);
        const firstChar = slotRefStr.charAt(0);
        if (firstChar === '\'' || firstChar === '"') {
          slotRefStr = slotRefStr.substring(1, slotRefStr.length - 2);
        }
